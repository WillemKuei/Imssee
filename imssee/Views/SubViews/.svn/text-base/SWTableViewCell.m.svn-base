...
- (void)scrollViewPressed:(id)sender {
    if(_cellState == kCellStateCenter) {
        ...
        if ([_delegate respondsToSelector:@selector(swippableTableViewCellDidPressed:)]) {
            [_delegate swippableTableViewCellDidPressed:self];
        }
    }
}
...
- (void)scrollToLeft:(inout CGPoint *)targetContentOffset{
    targetContentOffset->x = [self rightUtilityButtonsWidth];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    if ([_delegate respondsToSelector:@selector(swippableTableViewCellBeginDragging:)]) {
        [_delegate swippableTableViewCellBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (targetContentOffset->x < [self rightUtilityButtonsWidth] / 2) {
        [self scrollToRight:targetContentOffset];
    }else if(targetContentOffset->x >[self rightUtilityButtonsWidth] / 2){
