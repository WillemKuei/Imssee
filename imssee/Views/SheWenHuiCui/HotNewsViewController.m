...
#define imgWidth 80
#define imgHeight 60
#define titleLableHeight 40
#define subTitleHeight 20

#import "HotNewsViewController.h"
#import "HSList.h"

@interface HotNewsViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    int _page;
}
...
{
    NSDictionary *params = @{@"pageNo":[NSNumber numberWithInt:_page],@"isTop":[NSNumber numberWithInteger:1]};
    [WGProgressHUD showHUDAddedTo:self.view status:@"请稍后"];
    __weak HotNewsViewController *blockSelf = self;
    [[HttpRequest sharedInstance] internationalNewsListWithParams:params completionBlock:^(id result) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==0) {
        ...
        view.frame = CGRectMake(0, 0, self.view.bounds.size.width, 150);
        ...
        UILabel *titleLable = [[UILabel alloc]initWithFrame:CGRectMake(0, view.frame.size.height-40, view.frame.size.width, 40)];
        titleLable.font = [UIFont systemFontOfSize:18];
        titleLable.textAlignment = NSTextAlignmentCenter;
        titleLable.text = new.title;
        titleLable.backgroundColor = [UIColor colorWithWhite:0.6 alpha:0.3];
        ...
        [imgView setImageWithURL:[NSURL URLWithString:imgPath] placeholderImage:[UIImage imageNamed:@"7"]];
        ...
        commentLable.frame = CGRectMake(self.view.bounds.size.width-commentLableWidth-10, cell_height-20-5, commentLableWidth, 20);
        [cell.contentView addSubview:commentLable];
        //标题
        UILabel *titleLable = [[UILabel alloc]initWithFrame:CGRectMake(imgView.frame.size.width+20, (cell_height-titleLableHeight-subTitleHeight)/2, self.view.frame.size.width-imgView.frame.size.width-timeLable.bounds.size.width-30, titleLableHeight)];
        titleLable.font = [UIFont systemFontOfSize:16];
        titleLable.numberOfLines = 0;
        titleLable.text = new.title;
        [cell.contentView addSubview:titleLable];
        //子标题
        UILabel *subTitleLable = [[UILabel alloc]initWithFrame:CGRectMake(imgView.frame.size.width+20, titleLable.frame.size.height+8, self.view.frame.size.width-imgView.frame.size.width-commentLable.bounds.size.width-30, subTitleHeight)];
